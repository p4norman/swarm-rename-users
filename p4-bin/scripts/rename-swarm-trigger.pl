#!/usr/bin/env perl
# Customized Swarm Trigger Script - adding "p4 renameuser" command trigger
# Perforce Swarm Trigger Script
#
# @copyright   2013-2020 Perforce Software. All rights reserved.
# @version     2019.3/1914134
#
...
my %config = (
    ...
    SWARM_TOKEN       => 'D393F586-87C6-718D-0E86-570C1B0C22B5',
    ADMIN_USER        => '',
    ADMIN_TICKET_FILE => '',
    P4_PORT           => '',
    P4                => 'p4',
...
error('Unknown or invalid argument provided') and usage('short')
    unless getopts('w:d:a:t:v:p:g:x:rc:ohzu:s:', \%args);

# Generate friendlier keys for the commonly used args
@args{qw(workspace cwd args type value p4port config_file exclude_group user server_version errorSeverity)} = @args{qw(w d a t v p c g u s x)};
...
if ($args{type} eq "userrename") {
    ...
    if ($arglist =~ /--to=(\S*)/){ $toname = $1; }

    if ($fromname eq "" or $toname eq ""){ exit 0; } # need both from and to

    syslog(5, "renameuser  from: " . $fromname);
    syslog(5, "renameuser  to: " . $toname);

    $args{value} = $fromname;

    my @renames = ();
    push (@renames, { from => $fromname, to => $toname});

    my $json = JSON->new;
    $json_data = $json->encode (\@renames);
    syslog(5, "renameuser  json: " . $json_data);
}
...
sub get_trigger_entries {
    ...
    swarm.userrename command post-user-renameuser "$script$config -t userrename -x %maxErrorSeverity% -a %quote%%argsQuoted%%quote%"
...
Usage: $ME -t <type> -v <value> \\
    ...
    -x: command maxErrorSeverity, used by -x userrename 
